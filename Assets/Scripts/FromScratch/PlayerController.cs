...
#pragma warning restore 0414
...
#endif
#endif
                // initial line: Debug.LogFormat("local player {0} share anchors ", (CanShareAnchors ? "does not" : "does"));
        ...
        [Command]
        private void CmdDestroyBlock(GameObject blockUnit)
        {
            var blockUnitController = blockUnit.GetComponent<BlockUnitController>();
            var positionInMap = blockUnitController.positionInMap;
            // map を更新する
        ...
        public void SpawnBlockUnit(Vector3 localPos, Quaternion localRot, Color color, Vector3Int positionInMap)
        {
            if (isLocalPlayer)
            {
                CmdSpawnBlockUnit(localPos, localRot, color, positionInMap);
            }
        }

        [Command]
        private void CmdSpawnBlockUnit(Vector3 localPos, Quaternion localRot, Color color, Vector3 positionInMap)
        {
            GameObject blockUnit = Instantiate(SystemControllerInServer.Instance.blockUnitPrehab, localPos, localRot);
            var blockUnitController = blockUnit.GetComponent<BlockUnitController>();
            blockUnitController.color = color;
            blockUnitController.positionInMap = positionInMap;
            // map を更新する
            Vector3Int IpositionInMap = new Vector3Int((int)positionInMap[0], (int)positionInMap[1], (int)positionInMap[2]);
            SystemControllerInServer.Instance.blockCollectionMap[IpositionInMap[0]][IpositionInMap[1]][IpositionInMap[2]] = BlockCollectionData.colorToInt[color];
            NetworkServer.Spawn(blockUnit);
        }
